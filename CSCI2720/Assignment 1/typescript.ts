...
function scrolling() {
    let bar:HTMLElement|null = document.getElementById('Bar');
    if (bar != null)
        bar.setAttribute("style",'width: ' + window.scrollY/(document.documentElement.scrollHeight-window.innerHeight)*100 + '%');
};

function changeAlign(){
    let allsection:NodeListOf<HTMLElement>|null=document.querySelectorAll("h2");
...
function addHobby(){
    let hobby:string|null = prompt('Your new hobby here!');
    let hobbyList:HTMLElement|null = document.getElementById("hobbyList");
    if (hobby != null && hobby != '' && hobbyList != null)
        hobbyList.innerHTML += `<div class="m-2 p-3 bg-ternary">`+ hobby + `</div>`;
};

function displayScroll(){
    let sc:HTMLElement|null = document.getElementById('scroll');
    if (sc != null)
    {
